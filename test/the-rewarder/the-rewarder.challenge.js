...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        // Advance time 5 days to next round
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

        const attackerFactory = await ethers.getContractFactory('RewarderAttacker', attacker);
        this.attackContract = await attackerFactory.deploy(this.liquidityToken.address, this.rewardToken.address, this.flashLoanPool.address, this.rewarderPool.address);
        this.attackContract.connect(attacker).attack(TOKENS_IN_LENDER_POOL);
    });
